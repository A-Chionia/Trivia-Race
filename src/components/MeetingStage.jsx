...
export default function MeetingStage({user}) {
  ...
  const [fluidWorks, setFluidWorks] = useState(null);
    ...
    const receiveScore = useCallback((newScore) => {
      setScore(newScore);
...
return (
    <Fragment>
      ...
      {fluidWorks === false && <p style={textStyle}>There is an issue with your connection. Inform the other players and restart the application.</p>}
      {fluidWorks === true && <Unity unityProvider={unityProvider} style={{width:"100%"}} />}
    </Fragment>
  );
